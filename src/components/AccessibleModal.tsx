...
export default function AccessibleModal({ isOpen, onClose, title, children, className = '' }: AccessibleModalProps) {
  ...
  useEffect(() => {
    ...
    const focusable = rootRef.current?.querySelector<HTMLElement>('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])')
    focusable?.focus()
  ...
  if (!isOpen || !rootRef.current) return null
  ...
  return createPortal(modal, rootRef.current)
}
